...
- Archive/Trash messages like this builds a Gmail search from the sender plus up to four meaningful subject keywords, shows the matching thread titles for confirmation, then archives or trashes only after `Return`. `Esc` cancels the bulk action and moves to the next inbox message.
